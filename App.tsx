...
import { MAX_STORY_PAGES, BACK_COVER_PAGE, TOTAL_PAGES, INITIAL_PAGES, BATCH_SIZE, DECISION_PAGES, GENRES, TONES, LANGUAGES, ComicFace, Beat, Persona, UILanguage, UI_STRINGS } from './types';
...
const App: React.FC = () => {
  ...
  const [uiLanguage, setUiLanguage] = useState<UILanguage>('th'); // Default to Thai as requested
  
  // Update story language when UI language changes, but only if user hasn't manually selected a specific one?
  // Actually, let's keep them independent but maybe default story language to Thai if UI is Thai on first load.
  useEffect(() => {
     if (uiLanguage === 'th' && selectedLanguage === 'en-US') {
         setSelectedLanguage('th-TH');
     }
  }, [uiLanguage]);
  ...
  const handleAPIError = (e: any) => {
    ...
    console.error("API Error handled:", msg, e);
  ...
  const generateBeat = async (history: ComicFace[], isRightPage: boolean, pageNum: number, isDecisionPage: boolean): Promise<Beat> => {
    ...
    // Mix in custom premise if present, even for standard genres
    if (customPremise && customPremise.trim().length > 0) {
        coreDriver += ` ADDITIONAL PLOT CONTEXT/PREMISE: ${customPremise}.`;
    }
    ...
    1. UNLESS GENRE IS "Dark Sci-Fi" OR "Superhero Action" OR "Custom" OR PREMISE INVOLVES SCI-FI: DO NOT use technical jargon like "Quantum", "Timeline", "Portal", "Multiverse", or "Singularity".
...
`;
    try {
        ...
        handleAPIError(e);
        return { 
            caption: pageNum === 1 ? (selectedLanguage.includes('th') ? "จุดเริ่มต้น..." : "It began...") : "...", 
  ...
  const launchStory = async () => {
    ...
    if (selectedGenre === 'Custom' && !customPremise.trim()) {
        alert(uiLanguage === 'th' ? "โปรดระบุโครงเรื่องที่กำหนดเอง" : "Please enter a custom story premise.");
        return;
    }
  ...
  return (
    <div className="comic-scene">
      {showApiKeyDialog && <ApiKeyDialog onContinue={handleApiKeyDialogContinue} uiLanguage={uiLanguage} />}
