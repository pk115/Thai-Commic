...
import { GENRES, LANGUAGES, Persona, UILanguage, UI_STRINGS, GENRE_LABELS } from './types';
...
const Footer: React.FC<{ uiLanguage: UILanguage }> = ({ uiLanguage }) => {
  ...
  const remixIdeas = {
    ...
    th: [
        "เพิ่มเสียงประกอบในช่อง",
        "สร้างภาพเคลื่อนไหวด้วย Veo 3",
        "แปลเป็นภาษาคลิงออน",
        "เพิ่มเครื่องสร้างตัวร้าย",
        "พิมพ์ออกมาเป็นเล่มจริง",
        "เพิ่มเสียงบรรยาย",
        "สร้างจักรวาลที่เชื่อมโยงกัน"
    ]
  };

  useEffect(() => {
    const interval = setInterval(() => {
      setRemixIndex(prev => (prev + 1) % remixIdeas[uiLanguage].length);
    }, 3000);
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-black text-white py-3 px-6 flex flex-col md:flex-row justify-between items-center z-[300] border-t-4 border-yellow-400 font-comic">
        <div className="flex items-center gap-2 text-lg md:text-xl">
            <span className="text-yellow-400 font-bold">{UI_STRINGS[uiLanguage].remix}</span>
            <span className="animate-pulse">{remixIdeas[uiLanguage][remixIndex]}</span>
        </div>
...
export const Setup: React.FC<SetupProps> = (props) => {
    ...
    const S = UI_STRINGS[props.uiLanguage];
    ...
    return (
          ...
          <div className="min-h-full flex items-center justify-center p-4 pb-32 md:pb-24">
            ...
            <div className="max-w-[900px] w-full bg-white p-4 md:p-5 rotate-1 border-[6px] border-black shadow-[12px_12px_0px_rgba(0,0,0,0.6)] text-center relative">
                ...
                <div className="absolute top-0 right-0 p-2 z-50">
                    <button onClick={() => props.setUiLanguage(props.uiLanguage === 'en' ? 'th' : 'en')}
                            className="bg-black text-yellow-400 font-comic px-3 py-1 border-2 border-white hover:bg-gray-800 transition-colors">
                        {props.uiLanguage === 'en' ? '🇹🇭 THAI' : '🇺🇸 ENG'}
                    </button>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-4 text-left">
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                        <div className="font-comic text-xl text-black border-b-4 border-black mb-1">{S.castTitle}</div>
                        ...
                        <div className={`p-3 border-4 border-dashed ${props.hero ? 'border-green-500 bg-green-50' : 'border-blue-300 bg-blue-50'} transition-colors relative group`}>
                            <div className="flex justify-between items-center mb-1">
                                <p className="font-comic text-lg uppercase font-bold text-blue-900">{S.heroLabel}</p>
                                {props.hero && <span className="text-green-600 font-bold font-comic text-sm animate-pulse">{S.ready}</span>}
                            </div>
                            ...
                            {props.hero ? (
                                ...
                                <label className="comic-btn bg-blue-500 text-white text-lg px-3 py-3 block w-full hover:bg-blue-400 cursor-pointer text-center uppercase">
                                    {S.uploadHero} 
                                    <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && props.onHeroUpload(e.target.files[0])} />
                                </label>
                            )}
                        ...
                        <div className={`p-3 border-4 border-dashed ${props.friend ? 'border-green-500 bg-green-50' : 'border-purple-300 bg-purple-50'} transition-colors`}>
                            <div className="flex justify-between items-center mb-1">
                                <p className="font-comic text-lg uppercase font-bold text-purple-900">{S.costarLabel}</p>
                                {props.friend && <span className="text-green-600 font-bold font-comic text-sm animate-pulse">{S.ready}</span>}
                            </div>
                            ...
                            {props.friend ? (
                                ...
                                <label className="comic-btn bg-purple-500 text-white text-lg px-3 py-3 block w-full hover:bg-purple-400 cursor-pointer text-center uppercase">
                                    {S.uploadCostar} 
                                    <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && props.onFriendUpload(e.target.files[0])} />
                                </label>
                            )}
                    ...
                    <div className="flex-1 flex flex-col gap-2">
                        <div className="font-comic text-xl text-black border-b-4 border-black mb-1">{S.storyTitle}</div>
                        
                        <div className="bg-yellow-50 p-3 border-4 border-black h-full flex flex-col justify-between">
                            <div>
                                <div className="mb-2">
                                    <p className="font-comic text-base mb-1 font-bold text-gray-800">{S.genreLabel}</p>
                                    <select value={props.selectedGenre} onChange={(e) => props.onGenreChange(e.target.value)} className="w-full font-comic text-lg p-1 border-2 border-black uppercase bg-white text-black cursor-pointer shadow-[3px_3px_0px_rgba(0,0,0,0.2)] focus:outline-none focus:translate-x-[1px] focus:translate-y-[1px] focus:shadow-none transition-all">
                                        {GENRES.map(g => <option key={g} value={g} className="text-black">{GENRE_LABELS[g] ? GENRE_LABELS[g][props.uiLanguage] : g}</option>)}
                                    </select>
                                </div>

                                <div className="mb-2">
                                    <p className="font-comic text-base mb-1 font-bold text-gray-800">{S.languageLabel}</p>
                                ...
                                <div className="mb-2">
                                    <p className="font-comic text-base mb-1 font-bold text-gray-800">{S.premiseLabel}</p>
                                    <textarea value={props.customPremise} onChange={(e) => props.onPremiseChange(e.target.value)} placeholder={S.premisePlaceholder} className="w-full p-1 border-2 border-black font-comic text-lg h-20 resize-none shadow-[3px_3px_0px_rgba(0,0,0,0.2)]" />
                                </div>
                            </div>
                            ...
                            <label className="flex items-center gap-2 font-comic text-base cursor-pointer text-black mt-1 p-1 hover:bg-yellow-100 rounded border-2 border-transparent hover:border-yellow-300 transition-colors">
                                ...
                                <span className="text-black">{S.novelMode}</span>
                            </label>
                        </div>
                    </div>
                </div>
                ...
                <button onClick={props.onLaunch} disabled={!props.hero || props.isTransitioning} className="comic-btn bg-red-600 text-white text-3xl px-6 py-3 w-full hover:bg-red-500 disabled:bg-gray-400 disabled:cursor-not-allowed uppercase tracking-wider">
                    {props.isTransitioning ? S.launching : S.launch}
                </button>
            </div>
          </div>
        </div>
        ...
        <Footer uiLanguage={props.uiLanguage} />
        </>
    );
}
